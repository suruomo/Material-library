...
public class MetalServiceImpl implements MetalService {
    ...
    public void uploadCard(MultipartFile file, String fileName) throws IOException {
        ...
        System.out.println(sheet.getLastRowNum());
        //获取数据库当前最大id编号
        int id = metalOutMapper.getMaxId();
        System.out.println("当前最大编号：" + id);
        //获取总列数
        int columnCount = sheet.getRow(0).getPhysicalNumberOfCells();
        //按行插入数据
        for (int r = 1; r <= sheet.getLastRowNum(); r++) {
            ...
            String form = row.getCell(4).toString();
            String temper = row.getCell(5).toString();
            String matId = row.getCell(6).toString();
            BigDecimal e = row.getCell(7) == null ? null : new BigDecimal(row.getCell(7).toString());
            BigDecimal eC = row.getCell(8) == null ? null : new BigDecimal(row.getCell(8).toString());
            BigDecimal nu = row.getCell(9) == null ? null : new BigDecimal(row.getCell(9).toString());
            BigDecimal g = row.getCell(10).toString() == null ? null : new BigDecimal(row.getCell(10).toString());
            BigDecimal fTu = row.getCell(11) == null ? null : new BigDecimal(row.getCell(11).toString());
            BigDecimal fTy = row.getCell(12) == null ? null : new BigDecimal(row.getCell(12).toString());
            BigDecimal fCy = row.getCell(13) == null ? null : new BigDecimal(row.getCell(13).toString());
            BigDecimal fSu = row.getCell(14) == null ? null : new BigDecimal(row.getCell(14).toString());
            BigDecimal fBru = row.getCell(15) == null ? null : new BigDecimal(row.getCell(15).toString());
            BigDecimal fBry = row.getCell(16) == null ? null : new BigDecimal(row.getCell(16).toString());
            BigDecimal rho = row.getCell(17) == null ? null : new BigDecimal(row.getCell(17).toString());
            BigDecimal a = row.getCell(18) == null ? null : new BigDecimal(row.getCell(18).toString());
            String books = row.getCell(19).toString();
            String remark = row.getCell(20).toString();
            String aircraft = row.getCell(21).toString();
            //赋值
            MetalOut metalOut = new MetalOut();
            metalOut.setTypeName(typeName);
            metalOut.setName(name);
            metalOut.setFamily(family);
            metalOut.setSpecification(specification);
            metalOut.setForm(form);
            metalOut.setTemper(temper);
            metalOut.setE(e);
            metalOut.seteC(eC);
            metalOut.setMatId(matId);
            metalOut.setBooks(books);
            metalOut.setRemark(remark);
            metalOut.setAircraft(aircraft);
            metalOut.setfBru(fBru);
            metalOut.setfBry(fBry);
            metalOut.setfCy(fCy);
            metalOut.setfSu(fSu);
            metalOut.setfTu(fTu);
            metalOut.setRho(rho);
            metalOut.setA(a);
            metalOut.setfTy(fTy);
            metalOut.setId(new BigDecimal(id));
            metalOut.setG(g);
            metalOut.setNu(nu);
            System.out.println("下一个id:" + id);
            //插入数据
